...
本小节先画**连续 chunk layout**下的容量映射，再补充常见非连续 layout；`fp16/fp32` 对应本文类型名 `f16/f32`。
...
```text
V<256xfp8> = [x0, x1, ..., x255]
```

物理视图（每个 BlockLane = 32B = 32 个 fp8 lane）：

```text
P0: 256B fp8 vreg
[BL0 0..31] [BL1 32..63] [BL2 64..95] [BL3 96..127]
[BL4 128..159] [BL5 160..191] [BL6 192..223] [BL7 224..255]
```

#### `V<256×fp16>`：2 个物理 reg

逻辑视图：

```text
V<256xfp16> = [x0, x1, ..., x255]
```

物理视图（每个 BlockLane = 32B = 16 个 fp16 lane）：

```text
P0: 256B fp16 vreg
[BL0 0..15] [BL1 16..31] [BL2 32..47] [BL3 48..63]
[BL4 64..79] [BL5 80..95] [BL6 96..111] [BL7 112..127]

P1: 256B fp16 vreg
[BL0 128..143] [BL1 144..159] [BL2 160..175] [BL3 176..191]
[BL4 192..207] [BL5 208..223] [BL6 224..239] [BL7 240..255]
```

非连续 `parity(EVEN/ODD)` layout（如 `DINTLV_B16` load 或 `vdintlv` 后保留奇偶态）：

```text
逻辑 V<256xfp16>
┌────┬────┬────┬────┬─────┬──────┬──────┬──────┬──────┐
│ x0 │ x1 │ x2 │ x3 │ ... │ x252 │ x253 │ x254 │ x255 │
└─┬──┴─┬──┴─┬──┴─┬──┴─────┴──┬───┴──┬───┴──┬───┴──┬───┘
  │    │    │    │            │      │      │      │
  │    │    │    │            │      │      │      └─ ODD
  │    │    │    │            │      │      └──────── EVEN
  ...
  └────────────────────────────────────────────────── EVEN
        ...
        ┌──────────────┴──────────────┐
        ▼                             ▼
P_even 256B fp16 reg              P_odd 256B fp16 reg
┌────┬────┬────┬─────┬──────┬──────┬──────┐
│ x0 │ x2 │ x4 │ ... │ x250 │ x252 │ x254 │
└────┴────┴────┴─────┴──────┴──────┴──────┘
┌────┬────┬────┬─────┬──────┬──────┬──────┐
│ x1 │ x3 │ x5 │ ... │ x251 │ x253 │ x255 │
└────┴────┴────┴─────┴──────┴──────┴──────┘

P_even ─┐
        ├─ INTLV_B16 ─▶ [x0][x1][x2][x3] ... [x254][x255]
P_odd  ─┘
```

#### `V<256×fp32>`：4 个物理 reg

逻辑视图：

```text
V<256xfp32> = [x0, x1, ..., x255]
```

物理视图（每个 BlockLane = 32B = 8 个 fp32 lane）：

```text
P0: 256B fp32 vreg
[BL0 0..7] [BL1 8..15] [BL2 16..23] [BL3 24..31]
[BL4 32..39] [BL5 40..47] [BL6 48..55] [BL7 56..63]

P1: 256B fp32 vreg
[BL0 64..71] [BL1 72..79] [BL2 80..87] [BL3 88..95]
[BL4 96..103] [BL5 104..111] [BL6 112..119] [BL7 120..127]

P2: 256B fp32 vreg
[BL0 128..135] [BL1 136..143] [BL2 144..151] [BL3 152..159]
[BL4 160..167] [BL5 168..175] [BL6 176..183] [BL7 184..191]

P3: 256B fp32 vreg
[BL0 192..199] [BL1 200..207] [BL2 208..215] [BL3 216..223]
[BL4 224..231] [BL5 232..239] [BL6 240..247] [BL7 248..255]
```

非连续 `chunk × parity(EVEN/ODD)` layout（典型来源：`V<256×fp16> -> V<256×fp32>` 加宽后保留 parity）：

```text
逻辑 V<256xfp32>
┌──────────────────── chunk 0 ────────────────────┐ ┌──────────────────── chunk 1 ────────────────────┐
│ [x0][x1][x2][x3] ... [x124][x125][x126][x127]   │ │ [x128][x129][x130][x131] ... [x252][x253][x254][x255] │
└───────────────────────┬─────────────────────────┘ └───────────────────────┬─────────────────────────┘
                        │ parity split                                      │ parity split
          ┌─────────────┴─────────────┐                       ┌─────────────┴─────────────┐
          ▼                           ▼                       ▼                           ▼
   C0.EVEN 256B fp32           C0.ODD 256B fp32         C1.EVEN 256B fp32           C1.ODD 256B fp32
┌────┬────┬────┬─────┬──────┐ ┌────┬────┬────┬─────┬──────┐ ┌──────┬──────┬──────┬─────┬──────┐ ┌──────┬──────┬──────┬─────┬──────┐
│ x0 │ x2 │ x4 │ ... │ x126 │ │ x1 │ x3 │ x5 │ ... │ x127 │ │ x128 │ x130 │ x132 │ ... │ x254 │ │ x129 │ x131 │ x133 │ ... │ x255 │
└────┴────┴────┴─────┴──────┘ └────┴────┴────┴─────┴──────┘ └──────┴──────┴──────┴─────┴──────┘ └──────┴──────┴──────┴─────┴──────┘

C0.EVEN ─┐                                      C1.EVEN ─┐
         ├─ INTLV_B32 ─▶ [x0][x1]...[x126][x127]         ├─ INTLV_B32 ─▶ [x128][x129]...[x254][x255]
C0.ODD  ─┘                                      C1.ODD  ─┘
```

#### `V<64×fp16>`：1 个 partial 物理 reg
...
物理视图（低 64 个 fp16 lane 有效；每个 BlockLane = 16 个 fp16 lane）：

```text
P0: 256B fp16 vreg
[BL0 0..15] [BL1 16..31] [BL2 32..47] [BL3 48..63]
[BL4 pad/undef] [BL5 pad/undef] [BL6 pad/undef] [BL7 pad/undef]
<----------- 128B logical payload -----------><-- 128B outside logical value -->
```

非连续 `part(EVEN/ODD)` partial layout（如单个 `V<64×fp32> -> V<64×fp16>` 窄化 carrier）：
...
└────┴────┴────┴─────┴─────┴──────┘
        ...
        │  part = EVEN
        ▼
EVEN carrier: 256B fp16 reg，128 个物理 fp16 lane 中偶数 lane 有效
phys lane  0    1    2    3          124  125  126  127
          ┌────┬───┬────┬───┬─────┬─────┬───┬─────┬───┐
        ...
        │  part = ODD
        ▼
ODD carrier: 256B fp16 reg，128 个物理 fp16 lane 中奇数 lane 有效
phys lane  0    1    2    3          124  125  126  127
          ┌───┬────┬───┬────┬─────┬───┬─────┬───┬─────┐
          │ _ │ x0 │ _ │ x1 │ ... │ _ │ x62 │ _ │ x63 │
          └───┴────┴───┴────┴─────┴───┴─────┴───┴─────┘

对比: fp8/i8 sub_part(P0~P3) 是 4B group 内 byte 槽位
          ┌────┬────┬────┬────┐ ┌────┬────┬────┬────┐
          │ P0 │ P1 │ P2 │ P3 │ │ P0 │ P1 │ P2 │ P3 │ ...
          └────┴────┴────┴────┘ └────┴────┴────┴────┘
```

#### `V<64×fp8>`：1 个 partial 物理 reg
...
连续 partial 物理视图（低 64 个 fp8 lane 有效；每个 BlockLane = 32 个 fp8 lane）：

```text
P0: 256B fp8 vreg
[BL0 0..31] [BL1 32..63]
[BL2 pad/undef] [BL3 pad/undef] [BL4 pad/undef]
[BL5 pad/undef] [BL6 pad/undef] [BL7 pad/undef]
<-- 64B logical payload --><----------- 192B outside logical value ----------->
```

如果该值来自 `V<64×fp32> -> V<64×fp8>` 的 `vcvt`，pto.as 会采用 `sub_part(P0)` 内部布局，
物理上不是把 64 个 fp8 连续放到低 64B，而是把每个 fp32 lane group 的第 0 个 byte 填上有效 fp8：

```text
P0: 256B ui8/fp8 carrier，按 64 groups × 4B 看
[grp0:  x0  _  _  _] [grp1:  x1  _  _  _] [grp2:  x2  _  _  _] ... [grp63: x63 _ _ _]
        P0  P1 P2 P3         P0  P1 P2 P3         P0  P1 P2 P3             P0  P1 P2 P3
```

该稀疏视图供 `vstore` lower 到 `PK4_B32` 时抽取，每个物理 reg 写出连续 64B fp8；
它是 lowering layout，不改变 `V<64×fp8>` 的逻辑视图。
...
`vload`/`vstore` 是逻辑访存。**`[mode]` 显式声明访问形态**,默认 `continuous`(连续);
...
> `[mode]` 与 layout 推断的关系:`[mode]` 是用户对**访问形态**的显式声明(怎么读/写 UB),
> layout 轴推断是 pto.as 对**寄存器侧摆放**的隐式决策(数据进入 vreg 后怎么摆)。两者正交:
> 即便 `[mode=continuous]`,pto.as 仍可能为服务下游 reduce 而把 load lower 成 `DINTLV_B*`
> (产生 `parity` 轴)——此时物理 dist 与 surface mode 不一致,是 pto.as 的合法优化。

**`vstore` 专用 mode**(compact 标量按 `group` 写出,见配套文档 §3.1):`{mode="ONE_PER_GROUP"}`
把 compact `V<C×T>` 的 `C` 个 scalar 分别写到 `C` 个连续地址,lower 到多条 `1PT_B*`。
该 mode 与上表 `[mode]` 同名空间,但语义专属于 compact store,不与 `continuous/unpack/dintlv/brc`
混用。

| op | Cat | In | Out | Datatypes |
|---|---|---|---|---|
| `vload` | A | `Ptr<T>`, `[mode]`, `[pmode]` | `V<L×T>` | i8–i32, f16/bf16, f32 |
| `vstore` | A | `V<L×T>`, `Ptr<T>`, `M`, `[mode]`, `[pmode]` | — | i8–i32, f16/bf16, f32 |

### `[mode]` 取值与指针类型 → 硬件 dist 推导

`[mode]` 默认 `continuous`;`B*` 后缀由 `Ptr<T>` 的元素位宽决定(`T` 为 8/16/32-bit → `B8`/`B16`/`B32`)。

| `[mode]` | 语义 | vload → 物理 dist | vstore → 物理 dist |
|---|---|---|---|
...
> 两入一出 store),物理上对应 `vldsx2`/`vstsx2`,但 surface 仍以单条 `vload`/`vstore` + `[mode=dintlv]`
> 表达,成对结果/输入由 pto.as 在 lowering 时展开。

示例:

```mlir
// 连续 load(默认 mode):UB → vreg
%v = pto.vmi.vload %ub[%offset] : !pto.ptr<f32, ub> -> !pto.vmi.vreg<64xf32>
//  ↑ pto.as:Ptr<f32> → B32,mode=continuous → pto.mi.vlds {dist="NORM"}
...
//  ↑ pto.as:Ptr<f32> → B32,mode=continuous → pto.mi.vsts {dist="NORM_B32"}

// 广播 load:标量/块复制进 vreg
%vb = pto.vmi.vload %ub[%offset] {mode = "brc"} : !pto.ptr<f32, ub> -> !pto.vmi.vreg<64xf32>
//  ↑ pto.as:Ptr<f32> → B32,mode=brc → pto.mi.vlds {dist="BRC_B32"}(每 lane = UB[base])

// 加宽 unpack load:窄源展开到宽 lane
%u = pto.vmi.vload %ub[%offset] {mode = "unpack"} : !pto.ptr<bf16, ub> -> !pto.vmi.vreg<64xf32>
//  ↑ pto.as:Ptr<bf16> → B16,mode=unpack → pto.mi.vlds {dist="UNPK_B16"}

// 解交织 load(成对 even/odd 半区;surface 单条,lowering 展开为 dual)
%lo, %hi = pto.vmi.vload %ub[%offset] {mode = "dintlv"}
    : !pto.ptr<f32, ub> -> !pto.vmi.vreg<64xf32>, !pto.vmi.vreg<64xf32>
//  ↑ pto.as:Ptr<f32> → B32,mode=dintlv → pto.vldsx2 "DINTLV_B32"

// 交织 store(两路源合成成对结果)
pto.vmi.vstore %lo, %hi, %ub_out[%offset], %mask {mode = "dintlv"}
    : !pto.vmi.vreg<64xf32>, !pto.vmi.vreg<64xf32>, !pto.ptr<f32, ub>, !pto.vmi.mask<b32>
//  ↑ pto.as:Ptr<f32> → B32,mode=dintlv → pto.vstsx2 "INTLV_B32"
