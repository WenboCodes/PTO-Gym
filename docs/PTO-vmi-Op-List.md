...
| VLane | 硬件 32B 原子归约单元，vreg = 8 VLane；VLane 容纳 `32B / bitwidth(T)` 个 lane |
...
`C` 必须整除 `L`；**每 sub-group 的 lane 数**为 `L/C`，其字节数 `W = (L/C)·bitwidth(T)/8` 决定与 VLane 边界的关系，从而决定 Category。

### 2.2 `group` → Category 决策表

记 `W = (L/C) · bitwidth(T) / 8`（一个 sub-group 的字节数，`L/C` 为每 sub-group 的 lane 数），`VLane = 32B`。

| `W` 与 VLane 关系 | Category | pto.as lowering | 物理指令 |
|---|---|---|---|
| `W == 32`（sub-group 恰为 1 VLane） | **B** | 每 VLane 独立 reduce，1 op per physical reg，无跨 reg 合并 | `vcgadd`/`vcgmax`/`vcgmin` |
| `W = k·32, k>1`（sub-group 跨 k 个 VLane，对齐 VLane 边界） | **B**（fold + vcg） | 先 `(k-1)` 次 `vadd/vmax/vmin` 把 k 个 VLane fold 成 1 个 VLane 宽的中间值，再 `vcg*` | `vmax×(k-1)` + `vcg*` |
| `W` 不是 32 的整数倍（sub-group 不对齐 VLane） | **C** | 物化成连续形态后用全向量 `vc*`，或 pto.as 选择 `NORM` load + 重排 | `vcadd`/`vcmax`/`vcmin`（物化后） |
...
`V<256×bf16>` 取 shared exponent，每 32-element block 一个 max → `256/32 = 8` 个 sub-group，故 `group=8`（每 sub-group `256/8 = 32` lane）。bf16 `bitwidth=16`，`W=32·16/8=64B=2 VLane` → 落 §2.2 第二行（`k=2`，对齐 VLane，Category B）。

- load：pto.as 从下游 `group=8` 逆推，选 `DINTLV_B16`（parity）让 even/odd 各成一 vreg。
- `vand` 提 exponent：A 类透传，两 parity vreg 各 `vand`。
- fold：`(k-1)=1` 次 `vmax` 把 even/odd 两 vreg fold 成 1 个，parity 轴消失，每 VLane 16 lane 覆盖一个 32-element block 的 max exponent。
- `vcgmax`：每 VLane 取 max → 8 个 shared exponent（与 `group=8` 一致；8 VLane 各产 1 个，一一对应）。
