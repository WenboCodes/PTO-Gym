...
**物理视图（连续）** — 每个 BlockLane = 32B = 32 个 fp8 lane
...
**物理视图（非连续, parity EVEN/ODD）** — K=1 时 parity 落在单 reg 内：偶数 lane 摆低 128B、奇数 lane 摆高 128B

```text
P0: 256B fp8 vreg
[BL0..BL3  EVEN: x0 x2 x4 ... x254]   ← 低 128B
[BL4..BL7  ODD:  x1 x3 x5 ... x255]   ← 高 128B
```
...
**物理视图（连续）** — 每个 BlockLane = 32B = 16 个 fp16 lane
...
```text
P0 (EVEN): 256B fp16 reg    P1 (ODD): 256B fp16 reg
[x0 x2 x4 ... x254]          [x1 x3 x5 ... x255]
```
...
**物理视图（连续）** — 每个 BlockLane = 32B = 8 个 fp32 lane
...
```text
P0 (chunk0 EVEN): [x0 x2 x4 ... x126]
P1 (chunk0 ODD):  [x1 x3 x5 ... x127]
P2 (chunk1 EVEN): [x128 x130 x132 ... x254]
P3 (chunk1 ODD):  [x129 x131 x133 ... x255]
```
...
**物理视图（连续）** — 低 64 个 fp16 lane 有效，每个 BlockLane = 16 个 fp16 lane
...
```text
EVEN carrier: [x0 _ x1 _ x2 _ ... x62 _ x63 _]   ← phys lane 0,2,...,126 有效
ODD  carrier: [_ x0 _ x1 _ x2 ... _ x62 _ x63]   ← phys lane 1,3,...,127 有效
```
...
**物理视图（连续）** — 低 64 个 fp8 lane 有效，每个 BlockLane = 32 个 fp8 lane
...
```text
P0: 256B fp8 carrier，按 64 groups × 4B 看
[grp0: x0 _ _ _] [grp1: x1 _ _ _] ... [grp63: x63 _ _ _]
