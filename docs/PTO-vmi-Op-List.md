...
| `[dist-mode]` | 可选访问形态（vload/vstore 专用），`{dist-mode = "continuous"\|"unpack"\|"dintlv"\|"brc"}`，默认 `continuous`；vstore 另有 `{dist-mode = "ONE_PER_GROUP"}`（见 Part 3） |
...
**`vstore` 专用 dist-mode**(compact 标量按 `group` 写出,见配套文档 §3.1):`{dist-mode="ONE_PER_GROUP"}`
把 compact `V<C×T>` 的 `C` 个 scalar 分别写到 `C` 个连续地址,lower 到多条 `1PT_B*`。
该 dist-mode 与上表 `[dist-mode]` 同名空间,但语义专属于 compact store,不与 `continuous/unpack/dintlv/brc`
混用。
