...
单个 vreg(V0)内部:256 lane 按 8 行 × 32 列行优先排列,
8 个 BlockLane 各 16 lane = 半行(一行的左/右各 16 element),每半行落一个 BlockLane:

  lane 0..15   = row0  c0..c15   ← BL0
  lane 16..31  = row0  c16..c31  ← BL1
  lane 32..47  = row1  c0..c15   ← BL2
  lane 48..63  = row1  c16..c31  ← BL3
       ...                        ...
  lane 224..239= row7  c0..c15   ← BL6
  lane 240..255= row7  c16..c31  ← BL7
```

> **与 `group=8` reduce 的对应**:`group=8` 把 256 lane 切成 8 组、每组 32 lane。在 8×32 行优先摆放下,每 32 lane 恰好是**完整的一行**(32 element)。因此一个 vreg 的 8 个 sub-group 就是它持有的 8 行(row 0–7),`vcgmax` 每行取一个 max → 8 个 shared exponent,与 8 行一一对应——这正是 MX block-scale "每 32-element block 一个 shared exponent"的语义(tile 的每一行即一个 32-element block)。
