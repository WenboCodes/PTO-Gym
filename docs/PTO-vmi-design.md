...
一个 32×32 的 bf16 tile 共 1024 个元素，需要 **4 个 `V<256×bf16>`**(每个 256 lane)。按行优先切成 4 个 32×8 列片,每个 vreg 装一片:lane `n·8 + j` = tile 行 `n`、列 `col_off + j`(`col_off ∈ {0,8,16,24}` 对应 V0~V3)。

```text
                32×32 bf16 tile
   col 0      col 7  col 8     col 15  col 16    col 23  col 24    col 31
┌──────────────┬──────────────┬──────────────┬──────────────┐
│ row0  c0..c7 │ row0  c8..c15│ row0 c16..c23│ row0 c24..c31│
│ row1  ...    │ row1  ...    │ row1  ...    │ row1  ...    │     ... 32 行
│ ...          │ ...          │ ...          │ ...          │
│ row31 c0..c7 │ row31 c8..c15│ row31 c16..c23│ row31 c24..c31│
└──────────────┴──────────────┴──────────────┴──────────────┘
   ←─ V0 ─→     ←─ V1 ─→     ←─ V2 ─→     ←─ V3 ─→
   32×8=256     32×8=256     32×8=256     32×8=256

单个 vreg(V0)内部:256 lane 按 32 行 × 8 列行优先排列,
8 个 BlockLane 各 16 lane = 2 行 × 8 列,每 2 行落一个 BlockLane:

  lane 0..7    = row0  c0..c7   ┐ BL0
  lane 8..15   = row1  c0..c7   ┘
  lane 16..23  = row2  c0..c7   ┐ BL1
  lane 24..31  = row3  c0..c7   ┘
       ...                        ...
  lane 240..247= row30 c0..c7   ┐ BL7
  lane 248..255= row31 c0..c7   ┘
```

> **与 `group=8` reduce 的对应**:`group=8` 把 256 lane 切成 8 组、每组 32 lane。在 32×8 行优先摆放下,每 32 lane 恰好是连续 4 行 × 8 列 = 32 element,即 tile 中连续 4 行的同一 8 列片。因此一个 vreg 的 8 个 sub-group 正好覆盖该 vreg 所拿 32 行里的 8 个"4 行段"(行 0–3、4–7、…、28–31),`vcgmax` 各取一个 max → 8 个 shared exponent,一一对应。
